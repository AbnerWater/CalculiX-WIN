				 ...
				 ITG *ndamp, double *dacon)
{
	...
	*nzl = 0;
	...
	else
	{
		...
		for (i = 0; i < *nmpc; i++)
		{
			ipompcold[i] = ipompc[i];
		}
		for (i = 0; i < 3 * *mpcend; i++)
		{
			nodempcold[i] = nodempc[i];
		}
		for (i = 0; i < *mpcend; i++)
		{
			coefmpcold[i] = coefmpc[i];
		}
		for (i = 0; i < 20 * *nmpc; i++)
		{
			labmpcold[i] = labmpc[i];
		}
		for (i = 0; i < *nmpc; i++)
		{
			ikmpcold[i] = ikmpc[i];
		...
		for (l = 0; l < ndatatot; l = l + *jout)
		{
	     ...
	     centrifugal and gravity loading or temperature) */
			...
			if (nherm == 1)
			{
				...
				else
				{
					...
					for (i = 0; i < nev; i++)
					{
						...
						for (j = 0; j < nev; j++)
						{
							am[(j + nev) * nev2 + i] = am[(j + nev) * nev2 + i] - cc[i * nev + j]; //*freq[l];
							am[j * nev2 + nev + i] = am[j * nev2 + nev + i] + cc[i * nev + j];	 //*freq[l];
						}
					}
			...
			else
			{
				...
				for (i = 0; i < nev; i++)
				{
					eig[2 * i] = sqrt(sqrt(d[2 * i] * d[2 * i] + d[2 * i + 1] * d[2 * i + 1]) + d[2 * i]) / sqrt(2.);
					eig[2 * i + 1] = sqrt(sqrt(d[2 * i] * d[2 * i] + d[2 * i + 1] * d[2 * i + 1]) - d[2 * i]) / sqrt(2.);
					if (d[2 * i + 1] < 0.)
						eig[2 * i + 1] = -eig[2 * i + 1];
				}
			}
						...
						for (j = 1; j < ndata; j++)
						{
							...
							else
							{
								freqnh[ndatatot + j] = (freqnh[ndatatot] + e[i]) / 2. +
													   (e[i] - freqnh[ndatatot]) * pow(fabs(y), 1. / bias) * y / (2. * fabs(y));
							}
						}
						ndatatot += ndata - 1;
					}
	...
	else
	{
		...
		*mpcend = mpcendold;
		for (i = 0; i < *nmpc; i++)
		{
			ipompc[i] = ipompcold[i];
		}
		for (i = 0; i < 3 * *mpcend; i++)
		{
			nodempc[i] = nodempcold[i];
		}
		for (i = 0; i < *mpcend; i++)
		{
			coefmpc[i] = coefmpcold[i];
		}
		for (i = 0; i < 20 * *nmpc; i++)
		{
			labmpc[i] = labmpcold[i];
		}
		for (i = 0; i < *nmpc; i++)
		{
			ikmpc[i] = ikmpcold[i];
